...
import { getKeys } from 'apiSdk/keys';
...
function DashboardPage() {
  ...
  const keyStat = useSWR<PaginatedInterface<KeyInterface>>(
    () => `/keys`,
    () => getKeys({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Keys',
      result: keyStat?.data?.totalCount,
      isLoading: keyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Sessions',
